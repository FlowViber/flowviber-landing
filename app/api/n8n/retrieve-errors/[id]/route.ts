...
async function ensureDefaultUser() {
  ...
  try {
    ...
    if (result.rows.length > 0) {
      console.log("[v0] Retrieve Errors API: Default user found in database")
    ...
    console.log("[v0] Retrieve Errors API: Default user created in database")
    return defaultUserId
  } catch (error) {
    console.error("[v0] Retrieve Errors API: Error with default user:", error)
...
async function getUserId(): Promise<string> {
  ...
  console.log("[v0] Retrieve Errors API: Using default user ID:", defaultUserId)
  return defaultUserId
}
...
export async function GET(
  ...
  try {
    ...
    console.log("[v0] Retrieve Errors API: Retrieving errors for workflow:", workflowId)
    ...
    if (!n8nWorkflowId) {
      return NextResponse.json({ 
        success: true,
        errors: [],
        message: "Workflow not deployed to n8n yet",
    ...
    const errors = await n8nApi.getWorkflowErrors(n8nWorkflowId, userId)

    console.log("[v0] Retrieve Errors API: Retrieved", errors.length, "error executions")
    
    // Add detailed logging for error structure analysis
    if (errors.length > 0) {
      console.log("[v0] Retrieve Errors API: Sample error structure:", JSON.stringify(errors[0], null, 2))
    }
    ...
    try {
      ...
      console.log("[v0] Retrieve Errors API: Configuration test failed, but continuing with error retrieval")
    ...
    return NextResponse.json({
      ...
      hasConfiguration,
      message: errors.length > 0 ? `Found ${errors.length} errors` : "No errors found"
    })
  } catch (error) {
    console.error("[v0] Retrieve Errors API error:", error)
    return NextResponse.json(
