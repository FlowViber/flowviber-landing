...
export class ErrorAnalyzer {
  static analyzeExecutionError(error: any, workflowJson?: any): ErrorAnalysis {
    ...
    // Check if this is a configuration error (created by our validation)
    if (error.id?.startsWith('config-error-')) {
      console.log('[v0] ErrorAnalyzer: Detected configuration error')
      nodeId = error.id.replace('config-error-', '')
      nodeName = error.data?.resultData?.lastNodeExecuted || 'Unknown Node'
      errorMessage = error.data?.resultData?.error?.message || ''
      
      // For configuration errors, we can directly return the analysis
      if (errorMessage.includes('AI Agent node is missing a system prompt')) {
        return {
      ...
      if (errorMessage.includes('Missing') && errorMessage.includes('credentials')) {
        const credentialType = errorMessage.match(/Missing (\w+) credentials/)?.[1] || 'API'
        return {
          errorType: 'credential',
          nodeId,
          nodeName,
          nodeType: workflowJson?.nodes?.find((n: any) => n.id === nodeId)?.type || '',
          rootCause: 'Missing or invalid credentials',
          description: errorMessage,
          severity: 'critical',
          fixable: false,
          credentialIssue: {
            credentialType: credentialType,
            missingCredential: `${credentialType} credentials`,
            setupGuide: `Please add ${credentialType} credentials in your n8n instance`
          }
        }
      }
      
      if (errorMessage.includes('HTTP Request node is missing URL')) {
        return {
          errorType: 'configuration',
          nodeId,
          nodeName,
          nodeType: 'n8n-nodes-base.httpRequest',
          rootCause: 'Missing URL in HTTP Request node',
          description: 'The HTTP Request node requires a valid URL to make requests.',
          severity: 'high',
          fixable: true
        }
      }
      
      if (errorMessage.includes('Webhook trigger is not properly configured')) {
        return {
          errorType: 'configuration',
          nodeId,
          nodeName,
          nodeType: 'n8n-nodes-base.webhook',
          rootCause: 'Webhook trigger not configured',
          description: 'The Webhook trigger needs proper configuration to receive incoming requests.',
          severity: 'high',
          fixable: false
        }
      }
