...
export class ErrorBoundary extends React.Component<{ children: React.ReactNode }, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    ...
    if (this.state.mounted && typeof window !== "undefined") {
      try {
        const errorData = {
        ...
        console.error("[v0] Failed to store error:", storageError)
      }
    }
  }
  ...
  render() {
    // Don't render error UI on server side to avoid hydration issues
        ...
        <div className="min-h-screen bg-slate-900 text-white flex items-center justify-center p-4">
          <div className="max-w-md w-full bg-slate-800 rounded-lg border border-slate-700 p-6">
            ...
            <div className="flex gap-2">
              <Button onClick={this.handleReset} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white">
